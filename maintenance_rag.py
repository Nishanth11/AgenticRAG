...
class MaintenanceRAG:
    ...
    def __init__(
        ...
        if embedding_model == "local":
            ...
            # Fallback to Ollama if no other provider configured
        ...
        for doc in docs:
            ...
            if "Next Scheduled PM:" in content:
                ...
                for equipment, pm_date_str, duration in pm_matches:
                    ...
                    try:
                        # Extract just the date part (January 25, 2026)
...
def create_maintenance_tools(rag_system: MaintenanceRAG) -> List[Tool]:
    ...
    def check_maintenance_wrapper(input_str: str) -> str:
        ...
        # Parse input - expect format like "Product A, 2026-01-23"
    ...
    tools = [
        Tool(
            ...
            Input format: 'Product Name, YYYY-MM-DD' (e.g., 'Product A, 2026-01-23').
...
if __name__ == "__main__":
    ...
    print("\n1. Checking maintenance schedule for Product A on August 16, 2026:")
    result = rag.check_maintenance_schedule("Product A", "2026-08-16")
    ...
    result = rag.check_calibration_status("Product A", "2026-08-16")
    print(f"Can proceed: {result['can_proceed']}")
    if result['expired']:
